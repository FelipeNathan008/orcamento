...
@section('title', 'Editar Contato do Cliente: ' . $contatoCliente->cont_nome)

@section('content')
    <div class="max-w-6xl mx-auto p-8 mt-10 mb-10 font-poppins"> {{-- Contêiner principal para centralizar --}}
        {{-- Título centralizado, seguindo o exemplo --}}
        <h1 class="text-3xl font-bold text-custom-dark-text mb-8 text-center">Editar Contato:
        ...
        {{-- Removida a div com fundo branco para manter o estilo do create --}}
        <div>
            <form action="{{ route('contato_cliente.update', $contatoCliente->id_contato) }}" method="POST"
                class="space-y-6">
                @csrf
                @method('PUT') {{-- Método HTTP para atualização --}}

                {{-- Seção de campos do formulário em layout de duas colunas --}}
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="hidden" name="cliente_orcamento_id_co" id="selected_client_id_hidden"
                                value="{{ old('cliente_orcamento_id_co', $contatoCliente->cliente_orcamento_id_co) }}"
                                required>
                        ...
                        <div>
                            ...
                            <input type="text" name="cont_celular" id="cont_celular"
                                ...
                                placeholder="(XX) XXXXX-XXXX" maxlength="20"
                        ...
                        <div>
                            <label for="cont_telefone" class="block text-sm font-medium text-custom-dark-text mb-1">Telefone
                                (Opcional)</label>
                            <input type="text" name="cont_telefone" id="cont_telefone"
                                class="block w-full px-4 py-2 bg-white text-gray-900 placeholder-gray-400 rounded-md outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out border border-gray-300"
                                placeholder="(XX) XXXX-XXXX" maxlength="20"
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            <label for="cont_nome" class="block text-sm font-medium text-custom-dark-text mb-1">Nome do
                                Contato</label>
                            <input type="text" name="cont_nome" id="cont_nome"
                                class="block w-full px-4 py-2 bg-white text-gray-900 placeholder-gray-400 rounded-md outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out border border-gray-300"
                                placeholder="Nome completo do contato" maxlength="45"
                        ...
                        <div>
                            <label for="cont_email"
                                class="block text-sm font-medium text-custom-dark-text mb-1">Email</label>
                            <input type="email" name="cont_email" id="cont_email"
                                class="block w-full px-4 py-2 bg-white text-gray-900 placeholder-gray-400 rounded-md outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out border border-gray-300"
                                placeholder="contato@exemplo.com" maxlength="45"
                        ...
                        <div>
                            <label for="cont_tipo" class="block text-sm font-medium text-custom-dark-text mb-1">Tipo de
                                Contato</label>
                            <select name="cont_tipo" id="cont_tipo"
                                ...
                                <option value="administrativo" {{ old('cont_tipo', $contatoCliente->cont_tipo) == 'administrativo' ? 'selected' : '' }}>Administrativo
                                </option>
                                <option value="comercial" {{ old('cont_tipo', $contatoCliente->cont_tipo) == 'comercial' ? 'selected' : '' }}>Comercial</option>
                                <option value="financeiro" {{ old('cont_tipo', $contatoCliente->cont_tipo) == 'financeiro' ? 'selected' : '' }}>Financeiro</option>
                            </select>
                            @error('cont_tipo')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                ...
                <!-- Campo Descrição (Ocupando a largura total) -->
                <div class="pt-6">
                    <label for="cont_descricao" class="block text-sm font-medium text-custom-dark-text mb-1">Descrição
                        (Opcional)</label>
                    <textarea name="cont_descricao" id="cont_descricao" rows="4"
                        ...
                        maxlength="500">{{ old('cont_descricao', $contatoCliente->cont_descricao) }}</textarea>
    ...
    @push('scripts')
        ...
        <script>
            $(document).ready(function () {
                ...
                // NOVO: Pré-preenche o campo de busca com o nome do cliente atual
                const currentClientName = "{{ $contatoCliente->clienteOrcamento->clie_orc_nome }}";
                searchInput.val(currentClientName);
                // Também garante que o campo escondido já tem o ID correto
                hiddenInput.val("{{ $contatoCliente->cliente_orcamento_id_co }}");
