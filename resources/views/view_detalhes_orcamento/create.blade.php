{{-- resources/views/detalhes_orcamento/create.blade.php --}}
...
@section('content')
    <div class="max-w-6xl mx-auto p-8 mt-10 mb-10 font-poppins"> {{-- Contêiner principal para centralizar --}}
        {{-- Título centralizado --}}
        <h1 class="text-3xl font-bold text-custom-dark-text mb-8 text-center">Adicionar Novo Detalhe de Orçamento</h1>
        ...
        <div>
            <form action="{{ route('detalhes_orcamento.store') }}" method="POST" class="space-y-6" id="detalhesOrcamentoForm">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <select name="orcamento_id_orcamento" id="orcamento_id_orcamento"
                            ...
                            @foreach ($orcamentos as $orcamento)
                                <option value="{{ $orcamento->id_orcamento }}"
                                    {{ (old('orcamento_id_orcamento') == $orcamento->id_orcamento || (isset($selectedOrcamentoId) && $selectedOrcamentoId == $orcamento->id_orcamento)) ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="produto_id_produto" id="produto_id_produto"
                            class="block w-full px-4 py-2 bg-white text-gray-900 rounded-md outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out border border-gray-300"
                            required disabled> {{-- INICIALMENTE DESABILITADO --}}
                            <option value="">Selecione um Produto</option> {{-- Este será o placeholder padrão antes do Select2 --}}
                            @foreach ($produtos as $produto)
                                <option value="{{ $produto->id_produto }}"
                                    ...
                                    {{ old('produto_id_produto') == $produto->id_produto ? 'selected' : '' }}>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="det_cod" id="det_cod"
                                ...
                                placeholder="Código do detalhe" maxlength="45" value="{{ old('det_cod') }}" readonly>
                            @error('det_cod')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="det_modelo" id="det_modelo"
                                class="block w-full px-4 py-2 bg-gray-100 text-gray-700 rounded-md outline-none focus:ring-0 focus:border-gray-300 cursor-not-allowed border border-gray-300"
                                placeholder="Modelo do produto" maxlength="45" value="{{ old('det_modelo') }}" readonly>
                            @error('det_modelo')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="det_cor" id="det_cor"
                                class="block w-full px-4 py-2 bg-gray-100 text-gray-700 rounded-md outline-none focus:ring-0 focus:border-gray-300 cursor-not-allowed border border-gray-300"
                                placeholder="Cor do produto" maxlength="15" value="{{ old('det_cor') }}" readonly>
                            @error('det_cor')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="det_categoria" id="det_categoria"
                                class="block w-full px-4 py-2 bg-gray-100 text-gray-700 rounded-md outline-none focus:ring-0 focus:border-gray-300 cursor-not-allowed border border-gray-300"
                                placeholder="Categoria do detalhe" maxlength="45" value="{{ old('det_categoria') }}" readonly>
                            @error('det_categoria')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="det_valor_unit" id="det_valor_unit"
                                class="block w-full px-4 py-2 bg-gray-100 text-gray-700 rounded-md outline-none focus:ring-0 focus:border-gray-300 cursor-not-allowed border border-gray-300"
                                placeholder="Ex: 123,45" value="{{ old('det_valor_unit') }}" readonly>
                        ...
                        <div>
                            ...
                            <input type="text" name="det_genero" id="det_genero"
                                class="block w-full px-4 py-2 bg-gray-100 text-gray-700 rounded-md outline-none focus:ring-0 focus:border-gray-300 cursor-not-allowed border border-gray-300"
                                placeholder="Gênero do produto" maxlength="20" value="{{ old('det_genero') }}" readonly>
                            @error('det_genero')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="det_quantidade" id="det_quantidade"
                            class="block w-full px-4 py-2 bg-white text-gray-900 placeholder-gray-400 rounded-md outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out border border-gray-300"
                            placeholder="Quantidade do item" min="1" value="{{ old('det_quantidade') }}" required disabled>
                ...
                <div>
                    ...
                    <textarea name="det_observacao" id="det_observacao" rows="3"
                        class="block w-full px-4 py-2 bg-white text-gray-900 placeholder-gray-400 rounded-md outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out border border-gray-300"
                        placeholder="Informações adicionais sobre o detalhe do orçamento">{{ old('det_observacao') }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="det_anotacao" id="det_anotacao" rows="3"
                        class="block w-full px-4 py-2 bg-white text-gray-900 placeholder-gray-400 rounded-md outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out border border-gray-300"
                        placeholder="Anotações internas sobre o detalhe">{{ old('det_anotacao') }}</textarea>
                    @error('det_anotacao')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
    ...
    @push('scripts')
        ...
        <script>
            $(document).ready(function () {
                ...
                // Variáveis para armazenar os valores 'old' para persistência
                // Garante que oldDetTamanho seja um valor JS válido (string ou null)
                const oldDetTamanho = {!! json_encode(is_array(old('det_tamanho')) ? (old('det_tamanho')[0] ?? null) : old('det_tamanho')) !!};

                // Garante que oldCaractValues seja um array JS válido
                let oldCaractValues = [];
                const oldDetCaractRaw = {!! json_encode(old('det_caract')) !!};
                if (oldDetCaractRaw !== null && oldDetCaractRaw !== undefined) {
                 ...
                 $(document).ready(function() {
                    $('#orcamento_id_orcamento').select2({
                        placeholder: "Selecione um Orçamento",
                        allowClear: true,
                        width: '100%' // mantém o tamanho do select
                    });
                });
                ...
                // Função para limpar e desabilitar TODOS os campos dependentes
                function disableAndClearAllDependentFields() {
                    // Limpa e desabilita os campos imutáveis
                    immutableFields.forEach(field => {
                        field.val(''); // Limpa o valor
                        field.prop('readonly', true); // Mantém como readonly
                        // Assegura que as classes de estilo para "read-only" sejam aplicadas
                        field.removeClass('bg-white text-gray-900').addClass('bg-gray-100 text-gray-700 cursor-not-allowed');
                    });
                ...
                if (!orcamentoSelect.val()) {
                    // Se nenhum orçamento estiver selecionado no início,
                    // desabilita o select de produto e TODOS os campos dependentes.
                    produtoSelect.prop('disabled', true);
                    initializeProdutoSelect2("Selecione um Orçamento primeiro");
                    disableAndClearAllDependentFields(); // Chama para limpar e desabilitar tudo
                } else {
                    // Se um orçamento já estiver selecionado (e.g., via old() ou $selectedOrcamentoId),
                    // habilita o select de produto.
                    produtoSelect.prop('disabled', false);
                    initializeProdutoSelect2("Pesquise ou selecione um Produto");

                    // Se também houver um produto pré-selecionado,
                    // dispara o evento change para preencher e habilitar TODOS os campos editáveis.
                    if (produtoSelect.val()) {
                        produtoSelect.trigger('change');
                    } else {
                        // Se orçamento selecionado mas nenhum produto,
                        // mantém TODOS os campos dependentes desabilitados e limpos.
                ...
                produtoSelect.on('change', function() {
                    ...
                    if (productId) { // Se um produto válido for selecionado
                        ...
                        immutableFields.forEach(field => {
                            const dataAttr = field.attr('id').replace('det_', 'prod-'); // Converte 'det_cod' para 'prod-cod'
                            const value = selectedOption.data(dataAttr);
                            field.val(value);
                            // Aplica classes de read-only e fundo cinza
                        ...
                        if (prodPreco !== undefined && prodPreco !== null) {
                            ...
                            // Reaplicar a máscara após definir o valor
                            $('#det_valor_unit').mask('000.000.000.000.000,00', {reverse: true});
                        }
                        ...
                        let tamanhos = [];
                        let dataToParseTamanho = rawProdTamanhoAttr;

                        if (!dataToParseTamanho || dataToParseTamanho === "") {
                            dataToParseTamanho = '[]';
                        }

                        try {
                            tamanhos = JSON.parse(dataToParseTamanho);
                        ...
                        // Seleciona o valor 'old' para Tamanho
                        if (oldDetTamanho) {
                            detTamanhoSelect.val(oldDetTamanho);
                        }
                        ...
                        let caracteristicas = [];
                        if (prodCaractData) {
                            // Divide a string por vírgula e remove espaços em branco extras
                        ...
                        // Inicializa/re-inicializa o Select2 para o campo de característica
                        initializeDetCaractSelect2("Selecione uma Característica");

                        // Se houver um valor 'old' para det_caract, garante que ele seja selecionado após a população
                        if (oldCaractValues.length > 0) {
                            detCaractSelect.val(oldCaractValues).trigger('change'); // Define os valores selecionados e atualiza o Select2
                        } else {
                            detCaractSelect.val(null).trigger('change'); // Limpa a seleção se não houver old value
                        }

                    } else { // Se nenhum produto ou seleção limpa
                        // Limpa e desabilita todos os campos dependentes
                        disableAndClearAllDependentFields();
                    }
                });
                ...
                detCaractSelect.on('select2:selecting', function (e) {
                    ...
                    if ((isComBolso && hasSemBolsoAlready) || (isSemBolso && hasComBolsoAlready)) {
