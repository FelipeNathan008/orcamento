...
<div class="max-w-6xl mx-auto p-8 mt-10 mb-10 font-poppins">
    ...
    <form
        action="{{ route('produto.update', $produto->id_produto) }}"
        method="POST"
        class="space-y-6"
        id="produto-form"

        data-initial-familia="{{ old('prod_familia', $produto->prod_familia) }}"
        data-initial-categoria="{{ old('prod_categoria', $produto->prod_categoria) }}"
        data-initial-tamanhos='@json(old("prod_tamanho", $produto->prod_tamanho))'
        data-initial-caract="{{ old('prod_caract', $produto->prod_caract) }}"> @csrf
        @method('PUT')
...
<script>
    $(document).ready(function() {
        ...
        const initialFamilia = form.dataset.initialFamilia;
        const initialCategoria = form.dataset.initialCategoria;
        const initialCaract = form.dataset.initialCaract;

        let initialTamanhos = [];

        try {
            initialTamanhos = JSON.parse(form.dataset.initialTamanhos);
        } catch (e) {
            initialTamanhos = [];
        }
